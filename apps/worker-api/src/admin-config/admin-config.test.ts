...
import { setConfigValues, listEditableConfig, resetConfigValues, listAdminConfigAudit } from "./service";
...
function env(overrides: Partial<Env> = {}): Env {
  return { DB: new MemoryD1() as unknown as D1Database, INTERNAL_API_SECRET: "internal", ...overrides };
}

function request(): Request {
  return new Request("https://worker.local/internal/admin/config", { headers: { "x-admin-user": "test-admin", "x-request-id": "req-test" } });
}
...
describe("admin config encryption and service", () => {
  ...
  it("saves non-secret config and resets override", async () => {
    const e = env();
    ...
    const after = await listEditableConfig(e);
    expect(after.items.find((item) => item.key === "TELEGRAM_REAL_REVIEW_ENABLED")?.source).not.toBe("d1");
  });
  ...
  it("requires CONFIG_ENCRYPTION_KEY for secret saves and redacts audit", async () => {
    ...
    const saved = await setConfigValues(e, [{ key: "FIRECRAWL_API_KEY", value: "hidden-value" }], request());
    expect(saved.ok).toBe(true);
    const listed = await listEditableConfig(e);
    const secret = listed.items.find((item) => item.key === "FIRECRAWL_API_KEY");
    ...
    expect(audit.entries[0]).toMatchObject({ key: "FIRECRAWL_API_KEY", new_value_redacted: "[redacted]" });
  });
});
