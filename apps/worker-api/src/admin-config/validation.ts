...
export function validateAdminConfigValue(definition: AdminConfigDefinition, rawValue: unknown): AdminConfigValidationResult {
  ...
  switch (definition.valueType) {
    ...
    case "number": {
      if (!/^\d+(\.\d+)?$/.test(value)) {
        return { ok: false, error: "invalid_number", message: `${definition.key} must be a number.` };
      }
      const parsed = Number.parseFloat(value);
      if (!Number.isFinite(parsed)) {
        return { ok: false, error: "invalid_number", message: `${definition.key} must be a finite number.` };
      }
      if (definition.min !== undefined && parsed < definition.min) {
        return { ok: false, error: "value_too_small", message: `${definition.key} must be at least ${definition.min}.` };
      }
    ...
    case "url": {
      ...
      if (definition.preferHttps === true && parsed.protocol !== "https:" && !(definition.allowLocalHttp === true && local)) {
  ...
  if (value.startsWith("[")) {
    try {
      ...
      if (!Array.isArray(parsed) || parsed.some((entry) => typeof entry !== "string")) {
        return { ok: false, error: "invalid_model_chain", message: `${definition.key} must be a JSON array of model IDs or a comma-separated list.` };
      }
    ...
    entries = value.split(",").map((entry) => entry.trim()).filter((entry) => entry.length > 0);
  }

  if (entries.length > (definition.maxItems ?? 5)) {
    return { ok: false, error: "too_many_models", message: `${definition.key} supports at most ${definition.maxItems ?? 5} fallback models.` };
  }

  for (const entry of entries) {
    const safe = validateModelId(entry, definition.maxLength ?? 120);
...
function validateSafeString(definition: AdminConfigDefinition, value: string, secret: boolean): AdminConfigValidationResult {
  if (secret && value.length === 0) {
    return { ok: false, error: "empty_secret", message: `${definition.key} cannot be empty.` };
  }

  if (definition.key === "AI_MODEL") {
    return validateModelId(value, definition.maxLength ?? 120);
  }

  const maxLength = definition.maxLength ?? (secret ? 4096 : 512);
  if (value.length > maxLength) {
    return { ok: false, error: "value_too_long", message: `${definition.key} must be ${maxLength} characters or fewer.` };
  }
  if (/[\u0000-\u001f\u007f]/.test(value)) {
    return { ok: false, error: "invalid_control_character", message: `${definition.key} contains unsupported control characters.` };
  }
  return { ok: true, value };
}
...
function validateModelId(value: string, maxLength: number): AdminConfigValidationResult {
  if (value.length === 0) {
    return { ok: false, error: "empty_model_id", message: "Model ID cannot be empty." };
  }
  if (value.length > maxLength) {
    return { ok: false, error: "model_id_too_long", message: `Model ID must be ${maxLength} characters or fewer.` };
  }
  if (!/^[A-Za-z0-9._:\/-]+$/.test(value)) {
    return { ok: false, error: "invalid_model_id", message: "Model ID may contain only letters, numbers, dot, dash, underscore, slash, or colon." };
  }
  return { ok: true, value };
}
