...
export async function handleInternalTelegramOutputDebug(request: Request, env: Env): Promise<Response> {
  ...
  const itemRow = await env.DB.prepare(
    `SELECT id, canonical_url, text, author_handle, source, created_at, updated_at
       FROM items
      WHERE id = ?
      LIMIT 1`
  ...
  const promptRun = await env.DB.prepare(
    `SELECT id, item_id, generated_output_id, prompt_profile_id, status, model, error_message, created_at, updated_at
       FROM prompt_runs
  ...
  return jsonResponse({
    ...
    promptRun: promptRun ? {
      ...
      updatedAt: promptRun.updated_at
    } : undefined
  });
}
