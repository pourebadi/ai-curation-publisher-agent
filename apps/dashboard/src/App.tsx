...
type MainTab = "overview" | "setup" | "settings" | "integrations" | "safety" | "pilot" | "activity" | "technical";
...
function App(): JSX.Element {
  ...
  return (
    <main className="shell">
      ...
      {activeTab === "activity" && <AdminSettings client={client} enabled={internalReady} initialTab="audit" onNotice={setNotice} onRefreshStatus={refreshStatus} />}
      {activeTab === "pilot" && <section className="tabPanel"><div className="sectionTitle"><div><p className="eyebrow">Pilot Tests</p><h2>Safe checks only</h2></div></div><div className="buttonRow"><button type="button" onClick={() => void runOperation("pilot_readiness", () => client.runPilot({}), "Run readiness-only pilot?")} disabled={!internalReady || busy !== undefined}>Run readiness-only pilot</button><button type="button" onClick={() => void runOperation("scheduler_dry_run", () => client.runSchedulerDryRun(), "Run scheduler dry-run? This does not enable scheduler publishing.")} disabled={!internalReady || busy !== undefined}>Run scheduler dry-run</button></div><div className="grid three"><div className="subcard"><h3>Telegram review</h3><p className="muted">May contact Telegram only when backend review is configured. Does not final-publish.</p><label>Review text<textarea value={telegramText} onChange={(event) => setTelegramText(event.target.value)} /></label><label className="checkRow"><input type="checkbox" checked={confirmTelegram} onChange={(event) => setConfirmTelegram(event.target.checked)} />I understand this is review-only.</label><button type="button" onClick={() => void runOperation("telegram_review_dry_run", () => client.runTelegramReviewDryRun({ text: telegramText }), "Run Telegram review dry-run?")} disabled={!internalReady || !confirmTelegram || busy !== undefined}>Run review dry-run</button></div><div className="subcard"><h3>WordPress draft</h3><p className="muted">Creates/checks draft-only behavior. Does not public-publish.</p><label>Title<input value={wordpressTitle} onChange={(event) => setWordpressTitle(event.target.value)} /></label><label>Content<textarea value={wordpressContent} onChange={(event) => setWordpressContent(event.target.value)} /></label><label className="checkRow"><input type="checkbox" checked={confirmWordPress} onChange={(event) => setConfirmWordPress(event.target.checked)} />I understand this must remain draft-only.</label><button type="button" onClick={() => void runOperation("wordpress_draft_dry_run", () => client.runWordPressDraftDryRun({ title: wordpressTitle, content: wordpressContent }), "Run WordPress draft dry-run?")} disabled={!internalReady || !confirmWordPress || busy !== undefined}>Run draft dry-run</button></div><div className="subcard"><h3>Firecrawl sandbox</h3><p className="muted">May call an external provider only when enabled/configured. Does not publish anything.</p><label>URL<input value={firecrawlUrl} onChange={(event) => setFirecrawlUrl(event.target.value)} placeholder="https://example.com/article" /></label><label className="checkRow"><input type="checkbox" checked={confirmFirecrawl} onChange={(event) => setConfirmFirecrawl(event.target.checked)} />I understand this may call Firecrawl if enabled.</label><button type="button" onClick={() => void runOperation("firecrawl_sandbox_fetch", () => client.runFirecrawlSandboxFetch({ url: firecrawlUrl }), "Run Firecrawl sandbox fetch?")} disabled={!internalReady || !confirmFirecrawl || firecrawlUrl.trim().length === 0 || busy !== undefined}>Run sandbox fetch</button></div></div></section>}
...
function SetupWizardCards({ workerReady, internalReady, mode, encryptionReady, onOpen }: { workerReady: boolean; internalReady: boolean; mode: string; encryptionReady: boolean; onOpen: (tab: MainTab) => void }): JSX.Element {
  const locked = !workerReady;
  const steps = [
    { title: "1. Connect Worker", text: workerReady ? "Worker is reachable." : "Enter the Worker URL and check connection.", tab: "setup" as const, locked: false },
    { title: "2. Secure Admin Actions", text: internalReady ? "Admin secret is entered for this page." : "Enter INTERNAL_API_SECRET locally. It is not stored.", tab: "setup" as const, locked },
    { title: "3. Choose Operating Mode", text: mode === "manual_only" ? "Manual-only mode is active. Provider credentials are not required." : "Choose how content should enter the system.", tab: "settings" as const, locked },
    { title: "4. Configure AI", text: "Choose provider, model, output behavior, cost limits, and credentials.", tab: "settings" as const, locked },
    { title: "5. Configure Review Channel", text: "Set Telegram review details and credentials if review is needed.", tab: "integrations" as const, locked },
    { title: "6. Configure Publishing Drafts", text: "Set WordPress draft-only configuration. Public publishing stays disabled.", tab: "integrations" as const, locked },
    { title: "7. Optional Providers", text: mode === "manual_only" ? "Skipped for Manual-only unless you intentionally enable providers." : "Configure Firecrawl, Apify, or GetXAPI if needed.", tab: "integrations" as const, locked },
    { title: "8. Run Pilot Test", text: "Start with readiness-only. Each external-ish test asks for confirmation.", tab: "pilot" as const, locked },
    { title: "9. Launch Readiness", text: encryptionReady ? "Secret editing is available. Review safety before launch." : "Secret editing requires CONFIG_ENCRYPTION_KEY.", tab: "safety" as const, locked }
  ];
  return <div className="settingsList">{steps.map((step) => <div className={`settingRow ${step.locked ? "warning" : "safe"}`} key={step.title}><div><span className="configLabel">{step.locked ? "Locked" : "Available"}</span><strong>{step.title}</strong><p>{step.text}</p></div><div><button type="button" className="secondary" disabled={step.locked} onClick={() => onOpen(step.tab)}>Open</button></div></div>)}</div>;
}
