...
type MainTab = "overview" | "setup" | "settings" | "pilot" | "technical";
...
function App(): JSX.Element {
  ...
  const refreshStatus = useCallback(async (): Promise<void> => {
    ...
    setBundle(next);
    recordOperation("refresh_status", next.health?.ok === true && next.status?.ok === true, {
    ...
    <main className="shell">
      <header className="hero compactHero">
        <div>
          ...
          <p>Update safe runtime settings through the protected Worker API. Secrets are encrypted into D1 and never shown again.</p>
        </div>
        <div className="heroPanel">
          <strong>Security model</strong>
          <span>No Cloudflare API token in frontend</span>
          <span>No direct Worker Secret mutation</span>
          <span>No public publishing enablement</span>
        </div>
      </header>
      ...
      {activeTab === "overview" && (
        <section className="tabPanel">
          <div className="sectionTitle"><div><p className="eyebrow">Overview</p><h2>Safe admin status</h2></div><button type="button" onClick={() => void refreshStatus()} disabled={busy !== undefined}>Refresh</button></div>
          <div className={`launchStatus ${toneForOverall(setupCenter.launchSummary.overallStatus)}`}><strong>{setupCenter.launchSummary.overallStatus}</strong><span>{setupCenter.launchSummary.recommendedNextStep}</span></div>
          <div className="overviewCards">
            <OverviewCard title="Worker" value={workerReachable ? "Worker is online" : "Worker connection needed"} tone={workerReachable ? "safe" : "warning"} />
            <OverviewCard title="Admin secret" value={internalReady ? "Entered for this page" : "Required for editing"} tone={internalReady ? "safe" : "warning"} />
            <OverviewCard title="Secret encryption" value={encryptionEnabled === true ? "CONFIG_ENCRYPTION_KEY ready" : "Secret editing blocked"} tone={encryptionEnabled === true ? "safe" : "warning"} />
            <OverviewCard title="Scheduler" value={setupCenter.scheduler.riskLabel === "Safe" ? "Safe" : setupCenter.scheduler.riskLabel} tone={toneForRisk(setupCenter.scheduler.riskLabel)} />
            <OverviewCard title="Publishing" value={setupCenter.launchSummary.publishingSafety === "Safe" ? "Public publishing disabled" : "Risky config"} tone={setupCenter.launchSummary.publishingSafety === "Safe" ? "safe" : "risky"} />
            <OverviewCard title="Next action" value={internalReady ? "Open Settings to edit safe config." : "Enter INTERNAL_API_SECRET in Setup."} tone="plain" />
          </div>
        </section>
      )}
      ...
      {activeTab === "setup" && (
        <section className="tabPanel">
          <div className="sectionTitle"><div><p className="eyebrow">Setup</p><h2>Connect dashboard to Worker</h2></div></div>
          <p className="muted">The Worker URL is saved locally. The admin secret is kept in memory only and disappears when this page reloads.</p>
          ...
          <label>INTERNAL_API_SECRET<input value={credentialInput} onChange={(event) => setCredentialInput(event.target.value)} type="password" placeholder="Enter for this page session" /></label>
          <div className="buttonRow"><button type="button" onClick={saveLocalConnection}>Save locally</button><button type="button" onClick={() => void refreshStatus()} disabled={busy !== undefined}>Check connection</button><button type="button" className="secondary" onClick={clearLocalSettings}>Clear</button></div>
          <details className="subcard" open><summary>Where secrets are configured</summary><p><code>INTERNAL_API_SECRET</code> and <code>CONFIG_ENCRYPTION_KEY</code> must be configured as Cloudflare Worker Secrets. Use:</p><pre>pnpm wrangler secret put CONFIG_ENCRYPTION_KEY</pre><p>Integration credentials can be added or rotated in Settings after encryption is configured.</p></details>
        </section>
      )}
      ...
      {activeTab === "technical" && (
        <section className="tabPanel">
          <div className="sectionTitle"><div><p className="eyebrow">Technical Details</p><h2>Debugging</h2></div><button type="button" className="secondary" onClick={() => { clearOperationHistory(); setHistory([]); }}>Clear history</button></div>
          ...
          <details className="subcard"><summary>Recent operation results</summary><RecentResults history={history} /></details>
          <details className="subcard"><summary>Non-editable protected settings</summary><ul><li>INTERNAL_API_SECRET</li><li>CONFIG_ENCRYPTION_KEY</li><li>CLOUDFLARE_API_TOKEN</li><li>CLOUDFLARE_ACCOUNT_ID</li><li>D1 database id and deployment credentials</li></ul></details>
        </section>
      )}
    </main>
  );
...
function RecentResults({ history }: { history: OperationRecord[] }): JSX.Element {
...
function resultToJson(result: ApiResult | undefined): JsonValue {
  ...
  if (result.ok) return result.data as JsonValue;
...
function readBoolean(result: ApiResult | undefined, path: string[]): boolean | undefined {
  if (result?.ok !== true || !isRecord(result.data)) return undefined;
